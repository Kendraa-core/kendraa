...
DROP POLICY IF EXISTS "Admin users can update their institutions" ON institutions;
CREATE POLICY "Admin users can update their institutions" ON institutions
  FOR UPDATE USING ((select auth.uid()) = admin_user_id);
...
DROP POLICY IF EXISTS "Users can update their posted jobs" ON jobs;
CREATE POLICY "Users can update their posted jobs" ON jobs
  FOR UPDATE USING ((select auth.uid()) = posted_by);

-- Job applications table policies
DROP POLICY IF EXISTS "Applications viewable by applicant and job poster" ON job_applications;
CREATE POLICY "Applications viewable by applicant and job poster" ON job_applications
  FOR SELECT USING (
...
DROP POLICY IF EXISTS "Attendees viewable by organizer and attendee" ON event_attendees;
CREATE POLICY "Attendees viewable by organizer and attendee" ON event_attendees
  FOR SELECT USING (
...
DROP POLICY IF EXISTS "Profile views are viewable by profile owner" ON profile_views;
CREATE POLICY "Profile views are viewable by profile owner" ON profile_views
  FOR SELECT USING ((select auth.uid()) = profile_id);

DROP POLICY IF EXISTS "Authenticated users can record views" ON profile_views;
CREATE POLICY "Authenticated users can record views" ON profile_views
  FOR INSERT WITH CHECK ((select auth.uid()) IS NOT NULL);
